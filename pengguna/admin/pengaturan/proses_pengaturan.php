...
// Double-check role is admin (extra security layer)
...
// Sanitize full name (remove excessive whitespace)
...
try {
    ...
    if (!empty($password_baru) || !empty($konfirmasi_password) || !empty($password_lama)) {
        ...
        // VALIDATION 3e: New password should not be the same as old password (best practice)
        if ($password_lama === $password_baru) {
            $_SESSION['error_message'] = "Password baru tidak boleh sama dengan password lama. Silakan gunakan password yang berbeda untuk keamanan.";
            header('Location: pengaturan.php');
            exit;
        }
    ...
    // Update full_name in session if changed (IMPORTANT for header display)
    $_SESSION['full_name'] = $full_name;

    // ========================================
    // SET SUCCESS MESSAGE (Informative)
    // ========================================
    if ($password_changed) {
        $_SESSION['success_message'] = "Pengaturan akun berhasil diperbarui! Username, nama lengkap, dan password Anda telah diubah. Gunakan password baru untuk login selanjutnya.";
    } else {
        $_SESSION['success_message'] = "Pengaturan akun berhasil diperbarui! Username dan nama lengkap Anda telah diubah.";
    }

    // ========================================
    // LOG SUCCESSFUL UPDATE (Optional - for audit trail)
    // ========================================
    // Uncomment below if you want to log admin activities
    /*
    $log_message = "Admin '{$username}' (ID: {$id_user}) updated their profile.";
    if ($password_changed) {
        $log_message .= " Password was changed.";
    }
    ...
    */
    ...
    // ========================================
    // Log error for debugging (important for production monitoring)
    error_log("Database Error in proses_pengaturan.php (Admin): " . $e->getMessage());
    
    // User-friendly error message (don't expose database details)
    $_SESSION['error_message'] = "Terjadi kesalahan sistem saat memproses permintaan Anda. Silakan coba lagi atau hubungi administrator jika masalah berlanjut.";
    
    // For development/debugging only - show actual error (REMOVE IN PRODUCTION)
    ...
    // ========================================
    error_log("General Error in proses_pengaturan.php (Admin): " . $e->getMessage());
    $_SESSION['error_message'] = "Terjadi kesalahan yang tidak terduga. Silakan coba lagi.";
}
