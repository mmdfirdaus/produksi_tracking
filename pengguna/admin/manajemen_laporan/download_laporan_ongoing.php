...
// 1. Pengecekan sesi dan peran untuk 'admin'
if (!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== true || $_SESSION["role"] !== 'admin') {
    // Sebaiknya jangan redirect, cukup hentikan eksekusi karena ini adalah proses download
    http_response_code(403); // Forbidden
    die("Akses ditolak. Anda harus login sebagai admin.");
}

// 2. Path yang benar untuk file autoload dan koneksi dari direktori ini
...
// 3. Validasi input yang aman
$id_target = isset($_POST['id_target']) ? (int)$_POST['id_target'] : 0;
$bulan_laporan = isset($_POST['bulan_laporan']) ? $_POST['bulan_laporan'] : '';

if ($id_target === 0 || empty($bulan_laporan)) {
    die("Parameter tidak valid. Mohon kembali dan pilih target serta bulan laporan dengan benar.");
}
...
try {
    ...
    $centerAlignment = ['alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER, 'vertical' => Alignment::VERTICAL_CENTER]];
    ...
    foreach ($alurs as $alur) {
        ...
        for ($i = 1; $i <= $days_in_month; $i++) {
            ...
            $sheet->setCellValue(Coordinate::stringFromColumnIndex($colIndex) . $row, $i);
        }
        $highestColumnLetter = $sheet->getHighestColumn();
        $sheet->getStyle('A'.$row.':' . $highestColumnLetter . $row)->applyFromArray($tableHeaderStyle);
        $sheet->mergeCells('A' . $startRowTable . ':' . $highestColumnLetter . ($row-1));
        $row++;
        $dataStartRow = $row;
        ...
        $sheet->getStyle('A' . $dataStartRow . ':' . 'A' . ($row - 1))->applyFromArray($centerAlignment);
        $sheet->getStyle('C' . $dataStartRow . ':' . $highestColumnLetter . ($row - 1))->applyFromArray($centerAlignment);
    ...
    // 4. Header untuk memulai download
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment;filename="' . $filename . '"');
    header('Cache-Control: max-age=0');
    
    // Hapus output buffer yang mungkin ada sebelum mengirim file
    ob_end_clean();
    ...
    // Jangan tampilkan error database secara langsung di production
