...
try {
    ...
    // 4. TAB RINGKAS - Query dengan FIX (hanya dari target_material)
    $sql_ringkas = "
        SELECT
            tm.id_material,
            mk.nama_komponen,
        ...
        WHERE tm.id_target = :id_target
        GROUP BY tm.id_material, mk.nama_komponen, ma.nama_alur, ma.id_alur, kebutuhan_total, status_pengerjaan, ma.urutan
    ...
    $avg_progress = $total_kebutuhan_all > 0 ? round(($total_selesai_all / $total_kebutuhan_all) * 100, 1) : 0;

    // 5. TAB DETAIL - Query untuk bulan terpilih
    $sql_detail = "
         SELECT
            tm.id_material,
            mk.nama_komponen,
        ...
        WHERE tm.id_target = :id_target
        GROUP BY tm.id_material, mk.nama_komponen, ma.nama_alur, ma.id_alur, tm.jumlah_per_unit, pt.jumlah_unit, status_pengerjaan, ma.urutan
        ORDER BY ma.urutan ASC, mk.nama_komponen ASC
    ";
...
<div class="material-container">
    ...
    <div class="stats-container loading" style="animation-delay: 0.2s;">
        ...
        <div class="stat-card">
            ...
            <p class="stat-value"><?php echo $avg_progress; ?>%</p>
    ...
    <div class="nav-tabs-custom loading" style="animation-delay: 0.6s;">
        <button class="nav-link-custom active" onclick="switchTab('ringkas')">
            <i class="bi bi-list-ul"></i> Ringkas
        </button>
                                ...
                                <tbody>
                                    <?php $no = 1; foreach ($data_alur['items'] as $mat): ?>
                                        <?php
                                            ...
                                            $progress_class = $percentage >= 100 ? 'bg-success' : ($percentage > 70 ? 'bg-info' : ($percentage > 30 ? 'bg-warning' : 'bg-danger'));
                                        ?>
                                        <tr>
                                            ...
                                            <td class="text-center fw-bold <?php echo ($sisa <= 0) ? 'text-success' : 'text-danger'; ?>">
                                ...
                                <?php foreach ($data_alur['items'] as $mat): ?>
                                    ...
                                    <div class="mobile-card">
                                        ...
                                        <div class="mobile-card-row">
                                            ...
                                            <span class="mobile-value" style="color: <?php echo ($sisa <= 0) ? 'var(--success-color)' : 'var(--danger-color)'; ?>;">
                                                <?php echo number_format($sisa); ?>
                                            </span>
                                        </div>
                                        <div class="mobile-card-row">
                                ...
                                <table class="table-custom table-detail">
                                    <thead>
                                        <tr>
                                            ...
                                            <th rowspan="2" class="freeze-sisa-header">Sisa</th>
...
function navigateMonth(direction) {
    ...
    if (targetMonth) {
        window.location.href = `?id_target=${idTarget}&bulan=${targetMonth}#detail`;
    }
}
